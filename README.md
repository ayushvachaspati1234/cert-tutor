...
- Theme: cobalt blue + sunshine-yellow accents, polka-dot hero band, extra-rounded corners, Lexend font (`--ink`, `--accent`, `--gold` in `style.css`).
- Hero visual is a pure HTML/CSS **phone mock**: readiness ring (conic-gradient), streak chip, and an adaptive quiz card with a highlighted correct answer + explanation.
...
- Logo is an inline SVG mountain/checkpoint mark in `index.html` (nav + footer) — swap both `<svg class="logo-mark">` blocks to change it.
